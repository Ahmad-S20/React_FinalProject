import {Box, Button, CircularProgress, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Typography,Paper , Divider } from '@mui/material';
...
export default function Cart() {
  ...
  // 🟢 Handle empty cart case here
  if(!data?.items || data.items.length === 0){
    return (
      <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', mt: 5 }}>
        <Typography id="categories" variant="h5" sx={{ fontWeight: 700, mb: 2 }}>Your cart is empty 🛒</Typography>
        <Typography variant="body2" sx={{ color: 'text.secondary', mb: 3 }}>
          Looks like you haven’t added anything yet.
        </Typography>
        <Button variant="contained" component="a" href="/" sx={{ textTransform: 'none' }}>
          Continue Shopping
        </Button>
      </Box>
    );
    ...
    <Box sx={{ display: 'flex', gap: 2 }}>
      <Box sx={{ flex: 1 }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1, px: 1 }}>
          ...
          <Typography variant="body2" id="categories"  sx={{ color: 'text.secondary' }}>{`Items: ${data?.items?.length ?? 0}`}</Typography>
        </Box>
